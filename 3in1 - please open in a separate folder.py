...
def getMetaoblites(url_t):  #open the main page for study
    for i in url_t:
        StudyPage = urllib.request.urlopen(i).read().decode('utf-8')
        print(i)
        ...
        for i in mb_url_t:
            ...
            mb_regex = re.compile("MULTIPLE></td>[a-zA-Z0-9_()-±+',_\[\]α-ωΑ-Ω\s-]*?</td>",
                                  re.DOTALL | re.M)  # this is the best I can came up with so far that can find all metabolites
...
def main():
    ...
    x.write('\n'.join(mb_url_t))
    x.close()
    return(url_t,downloadlink_t,mb_test)
